...
object PStreamGenConversions {
  ...
  implicit def dstreamGen2batchMSeqGen[A](gs : Gen[PStream[A]]) : Gen[MSeq[Window[A]]] = gs.map(_.windows)
  implicit def dstreamGen2batchSeqGen[A](gs : Gen[PStream[A]]) : Gen[ISeq[Window[A]]] = gs.map(_.windows.toSeq)
...
class PStreamGen[A](self : Gen[PStream[A]]) {
   ...
   * */
  def ++(other : Gen[PStream[A]]) : Gen[PStream[A]] = UtilsGen.concMSeq(self, other)
