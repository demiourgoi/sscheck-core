...
object BatchGenConversions {
  ...
  implicit def batchGen2mseqGen[A](g : Gen[Window[A]]) : Gen[MSeq[A]] = g.map(_.points)
  implicit def batchGen2seqGen[A](g : Gen[Window[A]]) : Gen[Seq[A]] = g.map(_.points.toSeq)
  implicit def batchGen2iseqGen[A](g : Gen[Window[A]]) : Gen[ISeq[A]] = g.map(_.points.toSeq)
  
  implicit def iterableGen2batchGen[A](g: Gen[Iterable[A]]) : Gen[Window[A]] = g.map(Window[A](_))
  implicit def mseqGen2batchGen[A](g : Gen[MSeq[A]]) : Gen[Window[A]] = g.map(Window[A](_))
  implicit def iseqGen2batchGen[A](g : Gen[ISeq[A]]) : Gen[Window[A]] = g.map(Window.iseq2batch)
  implicit def seqGen2batchGen[A](g : Gen[Seq[A]]) : Gen[Window[A]] = g.map(Window.seq2batch)
}
...
class WindowGen[A](self : Gen[Window[A]]) {
   ...
   * */
  def ++(other : Gen[Window[A]]) : Gen[Window[A]] = UtilsGen.concMSeq(self, other)
